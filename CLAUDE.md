...
- New skills go in `skills/<name>/SKILL.md` — one level under `skills/`, never deeper. The **skill name is the folder name** and must be globally unique; it is the invocation identifier. Pick a `category:` from the 9 in [`skills/CATEGORIES.md`](skills/CATEGORIES.md) (or `featured`); recategorizing is a one-line frontmatter edit that moves nothing and breaks no links.
...
- Council perspectives are **lenses**, all in one file: `skills/clarity-council/LENSES.md` (decision lens, output requirements, blind spot per lens; an alias table; pre-made panels). Add a lens there — never a new file per persona. Callers name lenses the way they once named personas. The council itself is a skill (three inline modes: single / multi / iterative) and carries `category: featured`.
- The pack deliberately ships **no generic specialist or role-persona skills** — a capable model already covers generic expertise. A new skill must encode a procedure or judgment the model wouldn't produce unprompted; if it's only "act as an expert in X", it belongs as a lens in `LENSES.md`, not a skill.
- Keep `SKILL.md` to what every run needs. Material only some runs need (an optional mode, a conflict branch, worked examples, a report template) goes in a sibling `.md` the skill points to at the moment it's needed.
