...
- Council personas are reference documents (not skills — they have no `SKILL.md`) in the shared `skills/persona/` category. New personas go there. The `clarity-council` skill reads them; the council itself is a skill (three inline modes: single / multi / iterative). The `clarity-council` skill sits at the top level of `skills/` (not inside a category) as a featured skill many others delegate to.
