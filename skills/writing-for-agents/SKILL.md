...
description: Reference for writing any document an agent consumes — a skill, an AGENTS.md or CLAUDE.md, a doc reached by a pointer. Use when creating or editing a skill, writing agent-facing instructions, or modifying AGENTS.md / CLAUDE.md.
codex-short-description: "How to write documents agents consume: skills, AGENTS.md, pointer docs"
related-skills:
  - skill-create
  - which-skill
...
When the document you're writing is a skill, read [SKILL-MECHANICS.md](SKILL-MECHANICS.md) for frontmatter, the invocation choice, and router skills.
...
Before calling an agent-facing document finished, run it against its own levers:

1. **Pointer earns its load** — the description or `AGENTS.md` line names distinct branches, front-loads the leading word, and repeats nothing the body carries.
2. **Ladder is right** — what every branch needs is inline; what only some branches reach sits behind a pointer.
3. **Criteria are checkable** — every step ends on a bound the agent can tell done from not-done.
4. **No duplication** — each meaning lives in exactly one place, here or in the environment.
5. **No no-ops** — every sentence changes behaviour versus the model's default.
...
<!-- self-evolve:start -->
## Self-Evolve Loop
Journal `~/.ink-and-agency/learnings/writing-for-agents.md` (or workspace-local `.ink-and-agency/` where the sandbox confines writes). Read it first; append what the run taught — [SELF-EVOLVE.md](../SELF-EVOLVE.md).
<!-- self-evolve:end -->
