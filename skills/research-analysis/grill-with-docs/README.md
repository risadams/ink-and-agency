...
The skill walks the user through their plan one question at a time (same discipline as [grill-me](../../research-analysis/grill-me/)), but adds three behaviors: (1) it loads the project's `CONTEXT.md` and `docs/adr/` as context first; (2) it challenges terminology that conflicts with the documented glossary; (3) it updates `CONTEXT.md` inline when terms are sharpened, and offers ADRs when a decision meets a strict three-criteria bar (hard to reverse, surprising without context, result of a real tradeoff). Files are created lazily — only when there's something to write.
...
The skill follows the [grill-me](../../research-analysis/grill-me/) discipline (one question at a time, recommendation per question, codebase exploration over asking when possible) plus three additions:
