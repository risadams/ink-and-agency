...
The skill captures the idea(s) plus optional criteria, runs the relevant sibling skills (council always; grill if the idea is vague), scores every surfaced pro and con on Impact × Confidence, sums the weighted totals, and renders a structured table with a clear recommendation. It does **not** produce new options (use [idea-generate](../../research-analysis/idea-generate/)) and does **not** force a snap pick (use [idea-decision-maker](../../research-analysis/idea-decision-maker/)).
...
- ❌ **Use for snap decisions.** If you need a pick in five minutes, this is the wrong skill. Use [idea-decision-maker](../../research-analysis/idea-decision-maker/).
- ❌ **Use to generate ideas.** If you don't have the options yet, run [idea-generate](../../research-analysis/idea-generate/) first, then feed its output here.
