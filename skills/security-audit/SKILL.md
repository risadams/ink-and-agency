...
codex-short-description: "Source-first security audit: trust boundaries, verified findings, structured report"
related-skills:
  - penetration-tester
  - security-engineer
  - code-review
...
- **Output directory**: a new writable directory outside the target, defaulting to `~/.ink-and-agency/security-audit/<repo-name>/run-<N>`, where `<N>` is the next unused integer. Use a directory inside the target only when the user explicitly selects it and the parent verifies that version control ignores the whole directory. Otherwise stop and request an external path.
...
Vendored from [cloudflare/security-audit-skill](https://github.com/cloudflare/security-audit-skill) (MIT, © Cloudflare, Inc. — see [LICENSE](LICENSE)); the companion `.md` files, `report-schema.json`, and the `validate-*.cjs` validators ship unchanged. Only this file's frontmatter and default output directory were adapted for this pack. The validators need Node.js: `node validate-findings.cjs` and `node validate-coverage-ledger.cjs`.

<!-- self-evolve:start -->

## Self-Evolve Loop

Journal: `~/.ink-and-agency/learnings/security-audit.md` (workspace-local
`.ink-and-agency/learnings/security-audit.md` where the sandbox confines writes). Read it
