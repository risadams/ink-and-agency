...
| "invoke the `<name>` skill", `Skill:` blocks | Run the named skill. On Claude Code use the `Skill` tool; on other hosts, load and follow that skill's `SKILL.md` inline. Some skills (e.g. `code-review`, `work-plan`) spawn generic sub-workers to parallelize — if the host has no subagent capability, run those passes inline in sequence instead of failing. |
...
| Self-Evolve journal (`~/.ink-and-agency/learnings/*.md` / workspace `.ink-and-agency/learnings/*.md`) | Plain file reads/writes on any host — no host facility involved. Prefer the user-global path; where the sandbox confines writes to the workspace (Codex default `workspace-write`), use the workspace-local path. Contract: [SELF-EVOLVE.md](SELF-EVOLVE.md). |
...
Frontmatter fields beyond `name` and `description` (`allowed-tools`, `related-skills`, `loop-eligible`, `recurrence-hint`, `compatibility`, `codex-short-description`) are advisory metadata; hosts that don't understand them ignore them.
