...
A: No. Bases are read-only queries. To bulk-update notes (e.g., set `status: done` on many tasks), use the Obsidian Properties view or [obsidian-cli](../../obsidian/obsidian-cli/).
...
A: Yes — see the task tracker example. Filter by `file.hasTag("task")` and `status != "done"`, then group by status. Pair with [obsidian-markdown](../../obsidian/obsidian-markdown/) for the underlying task syntax.

## Related skills

- **[obsidian-vault](../../obsidian/obsidian-vault/)** — for creating the underlying notes that bases query.
- **[obsidian-markdown](../../obsidian/obsidian-markdown/)** — for writing the frontmatter properties that bases filter and display.
- **[obsidian-cli](../../obsidian/obsidian-cli/)** — for bulk-modifying properties across many notes (something bases can't do).
