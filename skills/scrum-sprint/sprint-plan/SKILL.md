...
This skill is the Claude port of the user's "Sprint Plan" prompt template. It expects [sprint-snapshot](../../scrum-sprint/sprint-snapshot/SKILL.md) has already produced a `start.canvas` for the target sprint. If it hasn't, the skill prompts to invoke `sprint-snapshot` first.
