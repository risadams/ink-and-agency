...
| which-skill | Router over the pack: describe a situation, get the one skill (or flow) that fits, why, and the exact command — the cure for remembering 55 skills. Reads FLOWS.md. `disable-model-invocation` — explicit invoke only |
