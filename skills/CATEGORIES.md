...
**80 skills across 9 categories**, plus the featured [`clarity-council`](clarity-council/) skill.

> **Featured:** [`clarity-council`](clarity-council/) carries `category: featured` — it's the
> multi-perspective engine many other skills delegate to. Its 31 decision lenses, the aliases for
> retired persona names, and the pre-made panels all live in one file,
> [`clarity-council/LENSES.md`](clarity-council/LENSES.md).

## Business & Product &nbsp;`business-product` (9)

Product delivery and issue management.
...
- [`implement`](implement/) — Build a piece of work (ticket, spec, or free-form request) through a tunable pipeline — test-first, verify, review, and track — where every phase c...
...
Git workflow, READMEs, assets, and procedure wizards.
...
- [`branch-resolve-conflicts`](branch-resolve-conflicts/) — Use when a git merge or rebase hits complex conflicts, or the user says "resolve conflicts" / "fix merge conflicts".
...
- [`visual-asset-generator`](visual-asset-generator/) — Use when you need to generate production-ready visual assets for a project — app icons, favicons, OG images, logos, wordmarks, or social media images.
...
- [`i-am-autistic`](i-am-autistic/) — Shape every response for an autistic reader — direct statements, required/optional labels, literal language, quantified uncertainty, named social s...
...
## Meta-Orchestration &nbsp;`meta-orchestration` (11)

Planning, handoff, research, teaching, and the plan-to-spec-to-tickets pipeline.
...
- [`plan-to-questionnaire`](plan-to-questionnaire/) — Turn a decision you can't answer alone into a Markdown questionnaire for the one person who can — filled in async, or worked through together in a...
...
- [`work-plan`](work-plan/) — Plan a huge chunk of work — more than one agent session can hold — as a shared map of decision tickets on your issue tracker, and resolve them one...
...
- [`domain-modeling`](domain-modeling/) — Actively build and sharpen a project's domain model — challenge fuzzy terms, stress-test with scenarios, and write CONTEXT.md and ADRs inline as de...
...
- [`writing-fragments`](writing-fragments/) — Mine the user for raw writing fragments — claims, vignettes, sharp sentences, half- thoughts — and append them to one document as material for a fu...
- [`writing-humanize`](writing-humanize/) — Remove signs of AI-generated writing — inflated symbolism, promotional language, vague attributions, em dash overuse, rule of three, AI vocabulary,...
- [`writing-shape`](writing-shape/) — Shape a file of raw material into an article through a conversational session — candidate openings, paragraph by paragraph, arguing format at each...
- [`writing-social-script`](writing-social-script/) — Generate a script for a dreaded social scenario — declining, asking for an extension, following up, setting a boundary — in two or three phrasings...
