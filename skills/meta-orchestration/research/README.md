...
- ❌ **Use it for questions about the current working directory.** For "how does *this* code work," the reading is local and foreground — reach for [codebase-explain](../../codebase-build/codebase-explain/) instead.
...
- **[codebase-explain](../../codebase-build/codebase-explain/)** — when the question is about the *local* code, not external docs/APIs; the reading is foreground and in-repo.
- **[defuddle](../../meta-orchestration/defuddle/)** — extract clean Markdown from a specific web page you already have the URL for (token-efficient reading of one source).
- **[work-plan](../../meta-orchestration/work-plan/)** — uses `/research` subagents to resolve its `research`-type decision tickets in parallel.
